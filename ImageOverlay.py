...
def main():
    ...
    #continue until user provides valid input that will allow the topPic to be fully displayed
    while x<halfW or x>800-halfW or y<halfH or y>600-halfH: 
        x=int(input("How far to the right do you want the image?"))
        y=int(input("How far to the left do you want the image?"))
        #if x and y are putside of range
        if x<halfW or x>800-halfW or y<halfH or y>600-halfH: 
            print("That is not a valid coordinate, try again") #error messges
        images(bg,gif,x,y)
